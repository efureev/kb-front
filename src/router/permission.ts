...
export function createPermissionGuard(router: Router) {
  ...
  router.beforeEach(async (to, from, next) => {
    ...
    */
    ...
    // eslint-disable-next-line no-console
    console.log('router.$isSSR', router.$isSSR)

    if (!router.$isSSR && !userStore.hasStatusHandled)
      await userStore.getLoginStatus()

    if (whitePathList.includes(to.path as PageEnum)) {
      // eslint-disable-next-line no-console
      console.log('to.path', to.path)
      if (to.path === LOGIN_PATH && userStore.isAuthenticated) {
        try {
          // await userStore.getLoginStatus()
    ...
    // if (!userStore.hasStatusHandled)
    //   await userStore.getLoginStatus()

    // const getLastUpdateTime = userStore.hasStatusHandled
    if (!userStore.isAuthenticated) {
      // eslint-disable-next-line no-console
